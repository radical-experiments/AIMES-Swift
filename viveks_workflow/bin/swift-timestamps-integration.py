...
if __name__ == "__main__":
    ...
    rejob = {'id'     : {'pattern' : 'JOB_START jobid=([\w-]+) tr',
             ...
             'host'   : {'pattern' : 'THREAD_ASSOCIATION jobid=%s[\w\s/:.\-=\+\[\]]*host=([\w.]*)',
                         ...
                         'flogs'   : None},
             'taskid' : {'pattern' : 'JOB_TASK jobid=%s\s+taskid=urn:(R-\d+[-,x]\d+[-,x]\d+[-,x]\d+)',
    ...
    retask = {'id'      : {'pattern': 'JOB_TASK jobid=[\w-]+\s+taskid=urn:(R-\d+[-,x]\d+[-,x]\d+[-,x]\d+)',
                           'lfilter': 'JOB_TASK jobid=',
                           'flogs'  : None},
              ...
              'blockid' : {'pattern': 'taskid=urn:%s\s+status=2\s+workerid=([\d\-]+):',
                           'lfilter': 'TASK_STATUS_CHANGE taskid=urn:R-\d+[-,x]\d+[-,x]\d+[-,x]\d+ status=2',
                           'flogs'  : None},
    ...
    for name, code in states['task'].iteritems():
        ...
        #                     'lfilter': 'TASK_STATUS_CHANGE taskid=urn:R-\d+[-,x]\d+[-,x]\d+[-,x]\d+ '+status+' workerid=[\d\-:]+',
        #                     'flogs'   : None}
        # else:
        status = "status=%s" % code
        retask[name] = {'pattern': DT+'TASK_STATUS_CHANGE taskid=urn:%s '+status,
                        'lfilter': 'TASK_STATUS_CHANGE taskid=urn:R-\d+[-,x]\d+[-,x]\d+[-,x]\d+ '+status+'$',
                        'flogs'   : None}
