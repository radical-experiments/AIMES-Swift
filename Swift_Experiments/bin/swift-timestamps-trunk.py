...
if __name__ == "__main__":
    ...
    rejob = {'id'     : {'pattern' : 'JOB_START jobid=([\w-]+) tr',
                         ...
                         'flogs'   : None},
             # BROKEN
             # JOB_START jobid=remote_driver-mnhoz5pm-0 tr=remote_driversleep 1800 tmpdir=bag_of_tasks-run002/jobs/r/remote_driver-mnhoz5pm-0
             'host'   : {'pattern' : 'THREAD_ASSOCIATION jobid=%s[\w\s/:.\-=\+\[\]]*host=([\w.]*)',
                         'lfilter' : 'THREAD_ASSOCIATION jobid=',
                         'flogs'   : None},
             # FIXED
             # JOB_TASK jobid=remote_driver-amhoz5pm-0 taskid=task-60
             'taskid' : {'pattern' : 'JOB_TASK jobid=%s\s+taskid=task-(\d+)',
             ...
             'start'  : {'pattern' : DT+'JOB_START jobid=%s',
                         ...
                         'flogs'   : None},
             # FIXED
             # JOB_TASK jobid=remote_driver-gmhoz5pm-0 taskid=task-57
             'task'   : {'pattern' : DT+'JOB_TASK jobid=%s taskid=task-%s',
                         'lfilter' : 'JOB_TASK jobid=',
                         'flogs'   : None},
             'end'    : {'pattern' : DT+'JOB_END jobid=%s',
              ...
              # FIXED
              # JOB_TASK jobid=remote_driver-amhoz5pm-0 taskid=task-60
    retask = {'id'      : {'pattern': 'JOB_TASK jobid=[\w-]+\s+taskid=task-(\d+)',
                           'lfilter': 'JOB_TASK jobid=',
                           'flogs'  : None},
              # FIXED
              # JOB_TASK jobid=remote_driver-gmhoz5pm-0 taskid=task-57
              'jobid'   : {'pattern': 'JOB_TASK jobid=([\w-]+)\s+taskid=task-%s',
                           'lfilter': 'JOB_TASK jobid=',
                           'flogs'  : None},
              # FIXED
              # taskid=task-54 status=16 workerid=0426-4410310-000000:000000
              'blockid' : {'pattern': 'taskid=task-%s\s+status=2\s+workerid=([\d\-]+):',
                           'lfilter': 'TASK_STATUS_CHANGE taskid=task-\d+ status=2',
                           'flogs'  : None},
              # FIXED
              # taskid=task-54 status=16 workerid=0426-4410310-000000:000000
              'workerid': {'pattern': 'taskid=task-%s\s+status=2\s+workerid=(%s:\d+)',
                           'lfilter': 'TASK_STATUS_CHANGE taskid=task-\d+ status=2',
                           'flogs'  : None}}
    ...
    for name, code in states['task'].iteritems():
        if code in [16, 2]:
            ...
            retask[name] = {'pattern': DT+'TASK_STATUS_CHANGE taskid=task-%s '+status+' workerid=%s',
                            'lfilter': 'TASK_STATUS_CHANGE taskid=task-\d+ '+status+' workerid=[\d\-:]+',
                            'flogs'   : None}
        else:
            status = "status=%s" % code
            retask[name] = {'pattern': DT+'TASK_STATUS_CHANGE taskid=task-%s '+status,
                            'lfilter': 'TASK_STATUS_CHANGE taskid=task-\d+ '+status+'$',
                            'flogs'   : None}
